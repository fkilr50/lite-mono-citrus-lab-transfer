...
# Set environment variable to avoid Intel OpenMP/MKL duplicate library error on Windows
...
def test_simple(args):
    ...
    assert (
        args.load_weights_folder is not None
    ...
    with torch.no_grad():
        for idx, image_path in enumerate(paths):
            ...
            print(
                "   Processed {:d} of {:d} images - saved predictions to:".format(
                    idx + 1, len(paths)
                )
            )
